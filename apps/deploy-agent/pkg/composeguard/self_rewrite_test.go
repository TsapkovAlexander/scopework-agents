...
// demoRoots — каталог приложения служит и корнем, и служебным каталогом: так
// агент вызывает проверку для стека, который поднимает из этого каталога.
var demoRoots = []string{"/opt/tracedocs/apps/demo"}
...
func TestBindOfOwnComposeIsRefused(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			v, err := Check([]byte(json), Options{Roots: demoRoots, ServiceDirs: demoRoots})
			if err != nil {
